...
`largeval_16k` config in `tools/bench-scan-regression.sh` (100k × 16 KiB
...
(`tools/bench-write-regression.sh`) only exercises **512 B values**
...
`tools/bench-write-regression.sh` mirroring the scan sentinel's
...
4. **Documentation** — the script header references
   `doc/working/write-flow-analysis.md` for the "Regression sentinel"
   section, but that file does not exist yet. Either create it with the
   large-value results section, or record the results in the script's
   reference block only (decide during implementation; the scan
   sentinel uses `doc/working/kv-scan-flow-analysis.md`, so a parallel
   `write-flow-analysis.md` is the consistent choice).

**Scope** (expected changed files):
- `tools/bench-write-regression.sh` — add `largeval_16k` config(s);
  record reference results in the reference block.
- `doc/working/write-flow-analysis.md` — create (if chosen) with a
  "Regression sentinel" section documenting the large-value write
...
**Acceptance**:
- `tools/bench-write-regression.sh` includes a `largeval_16k` config
