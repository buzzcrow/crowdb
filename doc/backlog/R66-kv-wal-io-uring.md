...
The crow-tree B-tree engine already has a mature io_uring reactor
(`lib/crow-tree/src/reactor.cpp`, `lib/crow-tree/include/crow-tree/reactor.h`)
that submits read/write/fsync SQEs on a dedicated io_uring ring and
dispatches CQE completions via callbacks. It is Linux-only (guarded by
`CROW_TREE_HAVE_LIBURING`), uses `liburing` (already in the pixi
...
(deferred to V2)"). The crow-tree reactor has since matured, making it
feasible to add a `Uring` variant now.

**Solution**: Add a `Uring` variant to `IoBackend` that reuses the
crow-tree reactor for WAL file I/O, eliminating `spawn_blocking` on the
durability path.

1. **`IoBackend::Uring` variant** — a new backend that wraps the
   crow-tree `Reactor` for WAL segment I/O. Selected at startup when
   ...
   - **Reactor sharing**: the WAL creates its own `Reactor` instance
     (one per store, not per group) — the reactor is a single-threaded
     io_uring event loop, and one reactor per store is sufficient
     (WAL I/O is not CPU-bound; it's syscall-bound). Alternatively, the
     WAL can share the crow-tree engine's reactor if a store has both
     (preferred — one reactor thread per store, serving both B-tree
     page I/O and WAL segment I/O). The design should evaluate both
     options and pick the simpler one.
...
2. **`WalFile` integration** — add `WalFileInner::Uring` variant that
   ...
   `truncate` via reactor SQE submission + CQE completion futures.
   The Rust FFI layer (`crow-tree-ffi`) exposes the reactor's submit
   API as async functions (returning a `Future` that resolves on CQE
   completion). The WAL's `WalFile` dispatches to these.

   - **Completion-based futures**: the reactor's CQE callback resolves
     ...
     no thread hop.
   - **Batched submission**: the reactor batches SQEs and submits them
     in a single `io_uring_submit` call, amortizing the syscall cost.
     The WAL's `pipeline_writer` already batches writes — the `Uring`
     backend can submit the entire batch as multiple SQEs in one
     `io_uring_submit`, then await all CQEs.
...
3. **`pipeline_writer` changes** — the pipeline writer's
   ...
   `spawn_blocking`. On the `Uring` backend, this becomes a reactor
   fsync SQE + CQE await — no thread hop. The `write_at` calls for
   record writes also become reactor write SQEs.

   - **No API change**: `WalFile::fdatasync` and `WalFile::write_at`
     remain `async fn` — the `Uring` variant just has a different
     internal implementation (reactor SQE vs `spawn_blocking`).

4. **Shared io_uring module** — extract the io_uring reactor
   abstraction into a shared module usable by both crow-tree and
   crow-kv WAL. Currently the reactor lives in `lib/crow-tree/src/
   reactor.cpp` and is C++ only. Options:

   - **Option A (preferred)**: expose the reactor's submit API via
     FFI (`crow-tree-ffi`), letting the Rust WAL call
     `reactor_submit_write` / `reactor_submit_fsync` / `reactor_submit_read`
     directly. The reactor is already C++ with a C-callable submit API
     (`reactor.h`'s `submit_read`/`submit_write`/`submit_fsync`). The
     FFI layer wraps these into Rust async functions. This reuses the
     mature C++ reactor without duplicating logic.
   - **Option B**: rewrite the reactor in Rust (using `io-uring` or
     `tokio-uring` crate). This is cleaner (pure Rust, no FFI) but
     duplicates the mature C++ reactor and adds a new dependency.
     Defer unless Option A proves awkward.

   Option A is preferred because the C++ reactor is already proven
...
- `lib/crow-kv/src/wal/wal_file.rs` — add `WalFileInner::Uring` variant
  ...
  `truncate` via reactor SQE submission.
...
- `lib/crow-kv/src/wal/pipeline_writer.rs` — no API change; the
  ...
  replacements.
- `lib/crow-tree/ffi/src/lib.rs` — expose reactor submit API as Rust
  async functions (`reactor_write` / `reactor_fsync` / `reactor_read`).
  These return a `Future` that resolves on CQE completion.
- `lib/crow-tree/include/crow-tree/reactor.h` — no change (submit API
  already exists: `submit_read` / `submit_write` / `submit_fsync`).
- `lib/crow-kv/src/wal/segment.rs` — no change (dispatches through
  `WalFile`).
- `lib/crow-kv/tests/wal/` — add tests for the `Uring` backend:
  - `fdatasync` via reactor resolves correctly (data durable after
    CQE).
  - `write_at` via reactor writes correct data at correct offset.
  - Batched writes + single fsync via reactor.
  - `O_DIRECT` alignment enforced.
  - Fallback to `File` when `liburing` not available (non-Linux).
  - Error injection: reactor CQE with error result propagates to
    `WalFile::fdatasync` caller.
- `doc/design/kv/design-crow-kv-wal.md` §4.6 — update to document the
  `Uring` variant and the reactor sharing model.

**Complexity**: Medium-High — the reactor submit API already exists in
C++ and is proven. The main work is the FFI bridge (exposing reactor
submit as Rust async functions) and the `WalFileInner::Uring` variant.
The `pipeline_writer` and `segment` layers need no changes (they
already use `async fn` interfaces). The hardest part is reactor
lifecycle management (creation, sharing with crow-tree, shutdown
ordering) and ensuring the CQE completion future integrates cleanly
with Tokio's polling model.

**Dependencies**: None (the C++ reactor and `liburing` already exist
in the build). This is a self-contained performance improvement.

**Alternatives considered**:

- **A: Rewrite reactor in Rust (`tokio-uring` crate).** Rejected for
  now — the C++ reactor is mature, tested, and handles `O_DIRECT`
  ...
  to eliminate the C++ reactor dependency for WAL-only deployments.
...
- **D: Defer to V2 (as originally planned).** Rejected — the crow-tree
  reactor is now mature, `liburing` is in the pixi environment, and
  the WAL's `IoBackend` abstraction is ready for a new variant. The
  performance win (eliminating thread hops on the durability path) is
  worth doing now.
...
- `WalFileInner::Uring` implements all `WalFile` operations via
  reactor SQE submission — no `spawn_blocking` on any WAL I/O path.
- `fdatasync` via reactor resolves only after the kernel confirms
  durability (CQE received).
...
- `pipeline_writer` and `segment` layers unchanged (drop-in async fn
  replacement).
- Reactor lifecycle: one reactor per store (shared with crow-tree if
  present), clean shutdown on store close.
