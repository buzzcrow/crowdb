...
hot-path findings, 5 medium findings, and 17 correct patterns. This item
tracks the fixes for the critical and medium findings; the OK findings
...
- `MetricsRegistry::register_*` does unsynchronized `push_back` on
  vectors that `flush_to()` iterates under `flush_mutex_` — a data race
  if a metric is registered concurrently with the background flush
  thread.
...
- A metric registered lazily after `start()` while the flush thread is
  running: concurrent `push_back` + iteration is a data race (UB).
  Expected after fix: registration is safe at any time, or enforced
  pre-start only with a clear error.
...
fix the metrics registry data race; lower-priority findings are
...
1. **`BufferPool::pin` — I/O outside the pool lock** —
   ...
   `flush_dirty()` (line 294, holds `mu_` while writing back all dirty
   frames). Highest-impact fix in the whole review. Consider a
   per-frame lock or lock-free hash table for the lookup if the
...
- Metrics registry frozen after start → a late registration must fail
  loudly, not silently drop the metric.
...
**MetricsRegistry (work item 4):**
- Register a metric concurrently with a running flush thread → no
  data race (run under ThreadSanitizer, verify clean). Unit test.
- Late registration after `start()` → either succeeds safely (mutex
  variant) or panics with a clear message (frozen variant). Unit test.

**thread_name_flag (work item 5):**
- `format()` on the spdlog backend with a concurrent
  `set_current_thread_name` → no data race (ThreadSanitizer clean).
  Unit test.

**Lower-priority items (work items 6–8):**
