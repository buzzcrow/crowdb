...
# CROWDB - Design: crowdb-tree Storage Engine (Overview)

Depends on: [`../kv/design-crowdb-kv.md`](../kv/design-crowdb-kv.md), [`../kv/design-crowdb-kv-state-machine.md`](../kv/design-crowdb-kv-state-machine.md)
Satisfies: [`../kv/design-crowdb-kv.md`](../kv/design-crowdb-kv.md) §8.3 (learner storage)

This is the parent document for **crowdb-tree**, the production storage engine that
backs CROWDB learners: an embeddable, ordered key-value engine implementing the
CROWDB `KVEngine` contract, built as a standalone C++ library (`libcrowdb-tree`) and
consumed from the Rust `crowdb-kv` crate over a C ABI. It records the decisions
...
- Consistent point-in-time read snapshots (`scan`, `compare`, snapshot export)
  ...
  reflects (§3.1).
- Compose with the **external** durability/GC flow: crowdb-tree exposes
  ...
  1. **Data-retention GC — logical, slot-driven.** Dropping tombstones and
     ...
     everywhere. crowdb-tree is *told* the floor via `set_gc_watermark`; the
  ...
  2. **Page reclamation — physical, internal.** Freeing B+tree pages and
     ...
     Crowdbtree-internal (epoch-based), automatic, **not** slot-driven.
...
- **Multi-version / MVCC time-travel reads.** Single version per key only —
  not worth the cost for CROWDB.
- **Tree-level range split/merge in v1.** This is *tree*-level (splitting a
  whole crowdb-tree into two), distinct from internal B+tree *page* split/merge
  (always done). One crowdb-tree per consensus group; partitioning happens at the
...
- **A second operation log.** crowdb-tree has no redo/replay log of its own.
  Durability of consensus output is the consensus WAL; crowdb-tree persists only
  its materialized state (a snapshot = immutable root + `last_applied_slot`).
  This lets crowdb-tree compose with the slot/WAL system (or any other log) for
  recovery.
...
during design. It is not linked and not a dependency. What crowdb-tree
...
crowdb-kv (Rust)
  PxLearner ──drives──► dyn KVEngine
                          ├─ InMemKV            (Rust, tests)
                          └─ CrowdbTreeEngine     (Rust, FFI adapter; §4)
                                │  C ABI: ct_open / ct_apply / ct_get / ct_scan / ct_get_async / ...
                                ▼
  libcrowdb-tree (C++)
  └─ Crowdbtree (one per consensus group)
...
- **One crowdb-tree per consensus group.** A node hosting many groups owns many
  lightweight `Crowdbtree` instances. Each owns its epoch manager, buffer pool,
...
- **Two-level write path.** `apply` writes into the concurrent **MemTable
  ...
  [`design-crowdb-tree-engine.md`](design-crowdb-tree-engine.md).
...
- **Async I/O.** A single-thread DiskIOUring per `Crowdbtree` handles demand-load
  reads and flush/snapshot writes via io_uring
  ([`design-crowdb-tree-engine.md §3`](design-crowdb-tree-engine.md#3-async-ffi-bridge)).
...
views; both `InMemKV` and `CrowdbTreeEngine` implement it:
...
carries an empty batch and leaves no MemTable entry, so crowdb-tree must **not**
...
The boundary is **coarse** (engine-level). The Rust `CrowdbTreeEngine`
(`lib/crowdb-kv/src/kv/crowdb_tree_engine.rs`) is a thin adapter that:
...
- Bridges async↔sync via the io_uring engine + completion-based `ct_future`
  ...
  Full protocol in [`design-crowdb-tree-engine.md §3`](design-crowdb-tree-engine.md#3-async-ffi-bridge).
- Maps C status codes to `EngineError`.

Ownership rules (enforced by convention, documented at the C API in
`lib/crowdb-tree/include/lib/crowdb-tree/c_api.h`, the single source of truth for exact
signatures):
...
| # | Decision | Rationale |
| --- | --- | --- |
| D1 | **Build crowdb-tree in C++** as `libcrowdb-tree`, consume from Rust over a C API. | The lowest storage layer (block device / RDMA) is C++. Placing the FFI boundary at the *top* (engine level) keeps the page-I/O hot path entirely in C++ and crosses FFI only at coarse `apply`/`get`/`scan`/`snapshot` calls. |
