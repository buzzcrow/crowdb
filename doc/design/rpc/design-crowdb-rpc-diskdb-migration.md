...
# CROWDB - Design: diskdb crowdb-rpc Service

Depends on: [`design-crowdb-rpc.md`](design-crowdb-rpc.md) §3 (Wire Format),
§4 (Control Plane), §6 (Flatbuffer Wrapper Convention);
[`design-crowdb-rpc-tcp.md`](design-crowdb-rpc-tcp.md) §2 (SocketTransport).
Satisfies: `design-crowdb-rpc.md` §4.4 (Server Side — Rust handler
dispatch), §6 (Flatbuffer Wrapper Convention — diskdb schema).

The diskdb service uses crowdb-rpc (flatbuffer over TCP). This doc covers
...
and rationale for the crowdb-rpc engine itself are in
`design-crowdb-rpc.md` §3–§6; this doc does not repeat them.
...
The diskdb server runs the crowdb-rpc server on `DISKDB_RPC_BASE` (in
`ports.rs`). The service registry stores the endpoint; clients use it
via `with_rpc_transport()`. The legacy `diskdb_service` and
`diskio_service` schemas are reserved in `crowdb-protocol`.
