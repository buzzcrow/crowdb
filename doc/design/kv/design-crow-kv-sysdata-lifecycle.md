...
# CROW - Design: Sysdata Lifecycle (ID Reuse Safety + Disk Move)

Depends on: `design-crow-kv-group0.md` (sysdata schema), `design-crow-kv-server.md` (HTTP mgmt API), `design-crow-diskdb.md` (disk status management), `design-crow-console.md` (console API)
Satisfies: `design-crow-kv-group0.md` §2.8 (hardware admin), `design-crow-kv-server.md` §2.4 (internal mgmt API), `design-crow-diskdb.md` §6 (Init-state zone load)
...
`CrowkvClient` wires the eviction:
...
`build_hardware_client` in `app/crow-web/src/mgmt.rs`:
...
`CrowkvClient` with that endpoint, and wraps it in a
...
`app/crow-cli/src/commands/disk_group.rs`.
...
`copy_disk_records` in `app/crow-web/src/lifecycle.rs`:
