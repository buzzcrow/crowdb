...
- Accept persists accepted state but does not advance the chosen or applied
  frontier. A matching Chosen notification advances the chosen frontier and
  wakes the background apply loop; missing or stale accepted values are
  recorded as gaps for recovery.
