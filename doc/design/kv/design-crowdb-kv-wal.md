...
# CROWDB - Design: Write-Ahead Log

Depends on: [`design-crowdb-kv.md`](design-crowdb-kv.md) §8.1, §8.2
Satisfies: [`design-crowdb-kv.md`](design-crowdb-kv.md) §8.1, §8.2

This document specifies CROWDB's write-ahead log. **There is exactly one durable
...
The crowdb-tree C++ storage layer has a similar but cleaner separation:
...
crowdb-tree side for this refactor.
...
This is repeated from design-crowdb-kv.md §8.1](design-crowdb-kv.md) because everything else in this section is a consequence of it. This is the **strict** mode. The **early-ack** mode (§5.4) relaxes the leader's own durability ordering but keeps the quorum-durability requirement; both modes are described in §5.4.
...
**No `DurableCommitWatermark` WAL record.** The state machine owns its own persistence boundary, but that boundary is the *engine's* durability (crowdb-tree's on-disk snapshot), not a separate state-machine-level snapshot file; see §6.2 below and `design-crowdb-kv-state-machine.md §2.1`. The WAL does not duplicate this information.
...
[`design-crowdb-kv-state-machine.md §2.1`](design-crowdb-kv-state-machine.md#21-state-machine-restart-engine-durability--wal-replay)
...
- **Optimization for a durable engine:** before Pass 2, the engine reports
  its own durable floor via `KVEngine::resume_from_slot()` (crowdb-tree's
...
the engine has already durably advanced past (crowdb-tree rejects such writes
outright; see `../tree/design-crowdb-tree-engine.md`).
...
(Steady-state gap repair below the frontier uses the same adopt-from-quorum logic, one slot at a time; see [`design-crowdb-kv-slot.md`](design-crowdb-kv-slot.md).)
...
Records with `slot < gc_slot` are eligible for GC. The two-watermark rule is justified in [`design-crowdb-kv-slot.md`](design-crowdb-kv-slot.md) §11.
...
4. Rebuild from peers via **snapshot install** ([§8.4 of design-crowdb-kv.md](design-crowdb-kv.md#84-snapshot-and-install)).

This is the **fail-out semantics** decided in design-crowdb-kv.md §8.1](design-crowdb-kv.md). We do not try to keep operating with partial WAL state on the surviving disks; that path requires per-slot replication across disks and adds disproportionate complexity.
