...
# CROWDB - Design: Test Strategy

Depends on: [`design-crowdb-kv.md`](design-crowdb-kv.md) §17
Satisfies: [`design-crowdb-kv.md`](design-crowdb-kv.md) §17
...
CROWDB is a distributed key-value store built on Paxos consensus. The test
...
```
store      (PxKvStore: many groups, one node identity, routing)   <- lib/crowdb-kv/tests/store.rs
  group    (PxGroup: 1 local + N remote replicas, full Paxos)      <- lib/crowdb-kv/tests/group.rs
    replica  (PxLocalReplica: acceptor + learner + WAL + slots)    <- lib/crowdb-kv/tests/replica.rs
      election (PxLocalReplica election state machine, pure logic) <- lib/crowdb-kv/tests/election.rs
      wal    (WalEngine: durable log)   slot (PxSlotList)           <- lib/crowdb-kv/tests/wal.rs, slot.rs
        unit (pure modules: codec, classifier, kv engine, roles)   <- lib/crowdb-kv/tests/{paxos,kv}.rs + wal/slot codec tests
deployment (crowdb-kv-server binary + HTTP mgmt API + multi-process)  <- app/crowdb-kv-server/tests/*
console    (mgmt API server + CLI: Axum REST, CLI commands)        <- app/crowdb-{web,cli}/tests/*
ui e2e     (Playwright browser: SPA + real backend)                <- app/crowdb-web/ui/e2e/*
```
...
| Binary | Layer | Drives |
| --- | --- | --- |
| `lib/crowdb-kv/tests/paxos.rs` | unit | `acceptor`, `learner`, `error` classifier, `roles` in isolation |
| `lib/crowdb-kv/tests/kv.rs` | unit | `InMemKV` / `KVEngine` / `CrowdbTreeEngine` apply semantics |
| `lib/crowdb-kv/tests/election.rs` | unit | `PxLocalReplica` election state machine (role transitions, vote granting, heartbeat, lease, term fencing) |
| `lib/crowdb-kv/tests/wal.rs` | subsystem | `WalEngine` append/replay/gc/segment + record codec + backends |
| `lib/crowdb-kv/tests/slot.rs` | subsystem | `PxSlotList` / `PxSlotNode` |
| `lib/crowdb-kv/tests/replica.rs` | replica | single `PxLocalReplica` (no peers) |
| `lib/crowdb-kv/tests/group.rs` | group | `PxGroup` multi-node clusters (real loopback crowdb-rpc, no mocks) |
| `lib/crowdb-kv/tests/store.rs` | store | `PxKvStore` routing / lifecycle / status / health |
| `app/crowdb-kv-server/tests/*` | deployment | server binary + HTTP API, multi-process clusters, CLI, startup |
| `lib/crowdb-kv-client/tests/*` | client e2e | `CrowdbClient` retry, topology cache, `NotLeaderHint` follow, `AnyReplica` read distribution + fallback against embedded crowdb-rpc servers |
| `app/crowdb-web/tests/*` | console mgmt API | Axum REST API server: node management, OpenAPI proxy, API forwarding |
| `lib/crowdb-console-shared/tests/*`, `app/crowdb-cli/tests/*` | console mgmt API | shared core (config, API client, health aggregation) + CLI commands |
| `app/crowdb-cli/tests/bench_benchmark.rs` | benchmark | `bench benchmark` lifecycle (deploy → run → collect → report → cleanup) + `bench compare` |
| `app/crowdb-web/ui/e2e/*` | UI E2E | Playwright browser tests: SPA interactions, context menus, dialogs, KV panel |
...
`PxGroup`, or `crowdb-kv-server` process must verify two things before
...
2. **Basic KV CRUD correctness via the client library.** After leader
   election, perform a minimal CRUD cycle through `CrowdbClient` (or
   ...
   **Edge-case coverage** (at least one test per layer must cover these):
   ...
   KV operations are crowdb-rpc only (no REST API for KV). Tests use
   `CrowdbClient` (deployment/UI layers) or `PxKvStore` public API
...
- **Client E2E** — `CrowdbClient` against embedded `PxKvStore` crowdb-rpc
  ...
  `crowdb-kv-server` binary is booted.
- **Deployment** — `crowdb-kv-server` binary + HTTP management API +
...
- **Console mgmt API** — Console management API server (Axum) + CLI
  ...
  crowdb-kv-server nodes, shared core (config, API client, health
  aggregation), CLI commands. KV correctness is delegated to the
  deployment or UI E2E layer.
- **UI E2E** — Playwright browser tests against a real `crowdb-web` +
  `crowdb-kv-server` backend: drives the SPA as an operator would (clicks,
...
## 6. crowdb-tree C++ Test Layers

The C++ crowdb-tree library (`libcrowdb-tree`) has its own test layers, separate
from the Rust test binaries above. They run as `test-tree-ct` in CI.

| Layer | Where | What it proves |
| --- | --- | --- |
| C++ unit | `liblib/crowdb-tree/tests/unit` | Single component correctness: cell encoding, leaf page build/read, delta replay, consolidation triggers, mapping table, epoch manager, split point. |
| C++ integration | `liblib/crowdb-tree/tests/integration` | Multi-component flows over `InMemoryPageStore` and `FilePageStore`: basic CRUD, batch apply, scan, split/merge, consistent view, snapshot roundtrip, GC. |
| Crash/recovery | `liblib/crowdb-tree/tests/recovery` | Durability: snapshot + recover, torn-page, superblock A/B, double-apply idempotency. Uses a `FaultyPageStore` with fault points for the FI matrix. |
| Rust FFI | `crowdb-kv` `tests/` | `CrowdbTreeEngine` trait conformance (shared parametrized tests with `InMemKV`), async bridge, buffer ownership, error mapping. |
| Cross-engine parity | `crowdb-kv` `tests/` | `InMemKV` and `CrowdbTreeEngine` produce identical state via `compare()` after random op streams, snapshot export/import round-trips, and mid-stream restart. |
...
1. **Unit invariants** from the matching test layer (property-based or
   deterministic).
2. **Failure-injection** matching [`design-crowdb-kv.md`](design-crowdb-kv.md) §9
   scenarios.
