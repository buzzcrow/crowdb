...
# CROWDB - Design: crowdb-kv Watch/Notify Extension

Depends on: [`design-crowdb-kv-rpc.md`](design-crowdb-kv-rpc.md) §3, §5, §6; [`design-crowdb-kv.md`](design-crowdb-kv.md) §3; [`design-crowdb-kv-state-machine.md`](design-crowdb-kv-state-machine.md) §2
Satisfies: [`design-crowdb-diskdb.md`](../diskdb/design-crowdb-diskdb.md) §8 (Follow-up — group-0 notify/watch), §15

This document covers the crowdb-kv watch/notify extension, the diskdb
...
- [1. WatchNotify crowdb-rpc Bidi Stream](#1-watchnotify-crowdb-rpc-bidi-stream)
  - [1.1 Why](#11-why)
...
- [7. Configuration](#7-configuration)
  ...
  - [7.2 crowdb-diskdb-client config](#72-crowdb-diskdb-client-config)
...
## 1. WatchNotify crowdb-rpc Bidi Stream

### 1.1 Why

crowdb-kv has no watch/notify capability without this extension. The only
...
`lib/crowdb-kv/src/rpc/fbs/kv_client.fbs` carries the watch/notify messages
...
- **Field numbers are append-only** — matches the RPC design's
  compatibility rule (`design-crowdb-kv-rpc.md` §5).
...
`lib/crowdb-kv/src/cluster/watch_registry.rs`:
...
`PxLearner` (`lib/crowdb-kv/src/paxos/learner.rs`) holds:
...
`PxGroup` (`lib/crowdb-kv/src/cluster/group.rs`) holds:
...
`lib/crowdb-kv-client/src/watch_notify.rs`:
...
/// Client for the crowdb-kv `WatchNotify` bidi stream.
...
a. `subscribe` resolves the group leader endpoint via `CrowdbClient`'s
...
b. Gets a crowdb-rpc connection from the shared `ConnectionPool`
...
- **`store_id` required** — the topology cache is keyed by
  `(store_id, group_id)`. Group 0 is `(0, 0)` (`CrowdbClient::SYSTEM_STORE`
...
- **Proactive topology refresh on reconnect** — the reader loop calls
  ...
  stream open.
- **`ConnectionPool` reuse** — `pool` is `pub(crate)` on `CrowdbClient`;
...
`Trigger` in `app/crowdb-diskdb/src/bg_task.rs`:
...
In `app/crowdb-diskdb/src/liveness/keepalive.rs`:
...
`app/crowdb-diskdb/src/liveness/notify.rs`:
...
a. On `run`, subscribe to the group-0 prefixes diskdb cares about.
   ...
   as `DISKDB_WATCH_PREFIXES` in `crowdb-protocol` (`key/diskdb.rs`):
   ...
   constant in `crowdb-protocol`, not the diskdb handler.
...
The `DiskdbClient` endpoint cache (`lib/crowdb-diskdb-client/src/client.rs`)
...
### 7.2 crowdb-diskdb-client config
...
`app/crowdb-diskdb/src/main.rs` startup sequence (additions):
...
3. On shutdown: the stop signal aborts the notify handler (its `run`
   ...
   compaction; crowdb-rpc + HTTP servers drain.

## 10. Test Design

### Unit tests (UT)

**WatchRegistry** (`lib/crowdb-kv/src/cluster/watch_registry.rs`):
...
**WatchNotify server handler** (`lib/crowdb-kv/src/rpc/kv_service.rs`):
...
**Trigger::TimerOrEvent** (`app/crowdb-diskdb/src/bg_task.rs`):
...
**Integration tests** (`lib/crowdb-kv/tests/group_test/watch_notify_test.rs`):
