...
- [x] **Define bounded unary messages**: replace the two legacy snapshot
...
- [x] **Add validated limits**: add snapshot chunk bytes, source-session
...
- [x] **Build source session actors**: create one bounded actor per admitted
...
- [x] **Serve session lifecycle RPCs**: Begin captures engine metadata and
...
- [x] **Instrument the source**: register active/capacity-rejected/expired/
...
- [x] **Add typed pull transport**: implement Begin, Read, Finish, and Abort
...
- [x] **Stream into the importer**: change `PxGroup::join_via_snapshot` to
...
- [x] **Make publication failure-safe**: keep the group local to
...
- [x] **Verify tail continuation**: after successful publication, wire the
...
- [x] **Instrument the receiver**: add imported bytes/chunks, reconnect
...
- [x] **Remove whole-buffer APIs**: delete `SnapshotReply`, `send_snapshot`,
...
- [x] **Remove legacy protocol claims**: delete `ESnapshotRequest`/
...
- [x] **Update permanent architecture**: describe bounded new-member transfer,
...
- [x] **Run focused and final gates**: execute all requirement commands, add a
