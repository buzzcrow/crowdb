...
//! E2E test for the crowdb-rpc transport (R115 migration).
//!
...
//! all RPCs go through the crowdb-rpc flatbuffer transport instead of
...
async fn diskdb_rpc_transport_e2e() {
    ...
    // 3. Start crowdb-diskdb subprocess (now with rpc_listen_addr).
    eprintln!("=== [rpc] starting crowdb-diskdb ===");
    let diskdb = DiskdbProcess::start(&cluster.mgmt_endpoints, false);
    diskdb.wait_for_ready().await;
    eprintln!(
        "crowdb-diskdb started: listen=127.0.0.1:{}, rpc=127.0.0.1:{}, http=127.0.0.1:{}",
        diskdb.listen_port, diskdb.rpc_port, diskdb.http_port
    );

    // 4. Build the DiskdbClient with crowdb-rpc transport enabled.
    ...
    // 5. Allocate 3 blocks via crowdb-rpc.
    ...
    // Query disk-group info via crowdb-rpc.
    let dg_info = client
    ...
    // Free the 3 blocks via crowdb-rpc.
    ...
    // 7. Recalc disk usage via crowdb-rpc.
    ...
    // 8. Compact + reclaim via crowdb-rpc.
    ...
    // 9. Trigger scan + get scan status via crowdb-rpc.
    ...
    // 10. Rebuild zone bitmap via crowdb-rpc.
