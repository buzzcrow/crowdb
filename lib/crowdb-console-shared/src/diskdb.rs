...
//!
//! These types mirror the JSON shapes returned by `crowdb-web`'s
//! `/api/diskdb/*` REST proxy (see `app/crowdb-web/src/diskdb.rs`).
...
// ── Response models (mirror the DTOs in crowdb-web/src/diskdb.rs) ────
