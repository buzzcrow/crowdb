...
pub use crowdb_protocol::{common_type::DiskGroupId, GroupId, NodeId, RackId, ReplicaId, StoreId};

use crate::snapshot::CrowdbTreeStatsSnapshot;
...
pub struct Node {
    ...
    pub host: String,
    pub ssh: SshCreds,
    /// 0 or 1 `crowdb-kv-server` per node, enforced console-side.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub server: Option<ServerProcess>,
}

/// Console's view of one deployed `crowdb-kv-server` process.
///
...
/// Lifecycle state of the `crowdb-kv-server` process (console's view).
...
/// What `crowdb-kv-server` reports for one local `PxStore` on one node.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct NodeStore {
    pub node_id: NodeId,
    pub store_id: StoreId,
    /// crowdb-rpc listen address (`host:port`) of this `PxKvStore`. Each store
...
pub struct LocalReplicaInfo {
    ...
    /// Mirrors `crowdb_kv`'s `KvStoreStatus::engine_healthy`, threaded through
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub crowtree_stats: Option<CrowdbTreeStatsSnapshot>,
