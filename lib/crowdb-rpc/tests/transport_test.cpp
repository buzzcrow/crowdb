...
TEST_F(TransportLoopbackTest, SendAndReceiveFrame)
{
    ...
    uint8_t buf[crowdb::rpc::HEADER_SIZE + 16];
    crowdb::rpc::serialize_header(buf, h);
    std::memset(buf + crowdb::rpc::HEADER_SIZE, 0xAB, 16);
...
TEST_F(TransportLoopbackTest, LargeDataPayloadDirectRead)
{
    ...
    std::vector<uint8_t> buf(crowdb::rpc::HEADER_SIZE + CTRL_SIZE + DATA_SIZE);
    crowdb::rpc::serialize_header(buf.data(), h);
    std::memcpy(buf.data() + crowdb::rpc::HEADER_SIZE, ctrl.data(), CTRL_SIZE);
    std::memcpy(buf.data() + crowdb::rpc::HEADER_SIZE + CTRL_SIZE, data.data(), DATA_SIZE);
