...
using crowdb::rpc::FrameParser;
using crowdb::rpc::FramingError;
using crowdb::rpc::Header;
using crowdb::rpc::HEADER_SIZE;
using crowdb::rpc::MAGIC;
using crowdb::rpc::parse_header;
using crowdb::rpc::serialize_header;
...
TEST(FramingTest, OneWayFlag)
{
    ...
    auto                 bytes = build_frame(2, ctrl.data(), 8, nullptr, 0, crowdb::rpc::FLAG_ONE_WAY);

    FrameParser parser;
    Frame      *frame = feed_all(parser, bytes);

    ASSERT_NE(frame, nullptr);
    EXPECT_EQ(frame->header.flags, crowdb::rpc::FLAG_ONE_WAY);
    delete frame;
}
