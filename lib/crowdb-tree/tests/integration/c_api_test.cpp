...
TEST(CApi, MemOpenApplyGetScan)
{
    ...
    uint64_t count   = 0;
    int32_t  trunc   = 0;
    ASSERT_EQ(ct_scan(t, nullptr, 0, nullptr, 0, 0, 0, nullptr, 0, 0, 0, 0, 0, 0, &entries, &count, &trunc), 0);
...
// Crowdbtree::encoded_op vector (key+pre-encoded cell) and call apply_encoded
...
{
    crowdb::tree_test::TempDir tmp("capi_");
...
{
    crowdb::tree_test::TempDir tmp("sync_");
    ASSERT_FALSE(tmp.path.empty());
    std::string path = tmp.path;
