...
/// One record of a multi-key batch passed to [`Crowdbtree::apply_batch`].
...
/// Opaque Rust handle keeping a [`Bytes`] alive while crowdb-tree borrows its
/// bytes via a C++ `kExternal` buffer (R30). Created by
/// [`Crowdbtree::apply_batch_external`]; freed by the C++ `drop_fn` callback
...
/// underlying `Arc` refcount. Called exactly once per handle by crowdb-tree when
...
/// One zero-copy op for [`Crowdbtree::apply_batch_external`] (R30). The value
/// [`Bytes`] is kept alive by a ref handle handed to C++; crowdb-tree borrows the
...
impl Crowdbtree {
    ...
    /// handle handed to C++; crowdb-tree borrows the value bytes via a C++
