...
use crate::{catalog::CatalogContext, error::ValidationError, namespace::NamespaceAuthority};

pub(super) struct Tokens(Hmac<Sha256>);
...
pub(super) fn binding(
    ...
    digest.update(b"crowdb-iceberg-table-list-v1");
    digest.update(context.catalog.as_bytes());
    digest.update(context.activation_epoch.to_be_bytes());
    digest.update(parent.namespace.as_bytes());
    digest.update(parent.name_epoch.to_be_bytes());
    digest.update(parent.identifier.encode()?);
    digest.update(
        u16::try_from(limit)
