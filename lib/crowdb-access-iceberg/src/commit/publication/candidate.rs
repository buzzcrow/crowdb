...
impl Publisher {
    pub(super) async fn write_candidate(
        ...
        let record = if let Some(value) = self.store.get(&key.encode()?).await.map_err(CatalogError::from)? {
            ...
            let mut reader = FileReader::new(self.blocks.clone(), *record.clone(), None, 16 * 1024)?;
        ...
        self.current(operation).await?;
        let selected = FileRepository::new(self.store.clone())
            .publish(operation.context, &record)
            .await?;
...
pub(super) fn response(head: &TableHead, canonical: &[u8]) -> Result<Vec<u8>, ValidationError> {
    let location =
