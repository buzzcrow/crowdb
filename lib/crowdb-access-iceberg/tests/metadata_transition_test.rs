...
fn upgrades_require_explicit_adjacent_steps_and_preserve_high_water_marks() {
    ...
    assert!(check(&prior, &candidate, &[2, 3], 10).is_ok());
    assert!(check(&prior, &candidate, &[3], 10).is_err());
    assert!(check(&prior, &candidate, &[], 10).is_err());
    assert!(check(&candidate, &prior, &[2, 1], 10).is_err());
...
fn historical_missing_lineage_is_readable_but_new_snapshots_require_it() {
    ...
    assert!(check(&prior, &prior, &[], 10).is_ok());
...
fn legacy_partition_counters_and_retained_summaries_cannot_be_reset() {
    ...
    assert!(check(&prior, &candidate, &[2], 10).is_err());
    candidate["last-partition-id"] = json!(1000);
    assert!(check(&prior, &candidate, &[2], 10).is_ok());
