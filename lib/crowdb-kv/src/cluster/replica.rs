...
//! [`PxReplicaError`], an in-process enum. The crowdb-rpc adapter
...
///
/// All [`ReplicaHandler`] and [`ReplicaClient`] methods return this. The crowdb-rpc
/// adapter (`crate::rpc::px_rpc_service`) maps to/from `PxReplicaError` at the
/// network boundary so `crowdb_kv` library code never names `PxReplicaError`
/// outside of `rpc/`.
...
///
/// All errors are transport-neutral ([`PxReplicaError`]); the crowdb-rpc adapter
