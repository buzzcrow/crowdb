...
// by CROWDB_HAVE_LIBURING, which crowdb-common/cpp/CMakeLists.txt defines only
// when liburing was found (never on macOS). Shared by the crowdb-tree btree
// page store and the diskio engine.
#pragma once

#ifndef CROWDB_HAVE_LIBURING
#    error \
        "crowdb-common/diskio_uring.h requires CROWDB_HAVE_LIBURING (liburing not found by CMake; io_uring is Linux-only)"
#endif
