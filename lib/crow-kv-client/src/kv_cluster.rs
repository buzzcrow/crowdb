...
//!
//! See `doc/design/kv/design-crow-kv-group0.md` §2.1, §2.2, §3.

use std::sync::Arc;
use std::time::{SystemTime, UNIX_EPOCH};

use crow_protocol::common::{GroupValue, ReplicaValue, StoreValue};
use crow_protocol::common_type::{GroupId, ReplicaId, StoreId};
use crow_protocol::key::{KvGroupKey, KvReplicaKey, KvStoreKey, TextKey};
...
impl KVClusterMetaClient {
    /// Wrap a `CrowkvClient` for group-0 KV-cluster topology access.
    #[must_use]
...
impl KVClusterAdmin {
    ...
    pub async fn list_stores(&self) -> Result<Vec<StoreSummary>> {
        let r: crow_protocol::mgmt::StoreListResponse = self.get_json("/stores").await?;
        Ok(r.stores)
    }
    ...
    pub async fn list_remote_replicas(
        ...
        let r: crow_protocol::mgmt::RemoteListResponse = self
