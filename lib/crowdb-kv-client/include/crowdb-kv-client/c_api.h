...
// C ABI for crowdb-kv-client (ffi feature): exposes HardwareClient and
// ServiceRegistryClient operations to C++ consumers (crowdb-diskio).
//
...
typedef void *crowdb_hw_client_t;
typedef void *crowdb_svc_client_t;

// ── Callback type ─────────────────────────────────────────────────

typedef void (*crowdb_kv_on_complete)(int status, const char *result_json, void *user_data);
...
void crowdb_hw_client_destroy(crowdb_hw_client_t client);
...
void crowdb_hw_list_owners(crowdb_hw_client_t client, crowdb_kv_on_complete callback, void *user_data);

// List all KV-group bind entries. Callback receives a JSON array
// of KVGroupBindEntry objects.
void crowdb_hw_list_binds(crowdb_hw_client_t client, crowdb_kv_on_complete callback, void *user_data);

// ── ServiceRegistryClient ─────────────────────────────────────────

// Create a ServiceRegistryClient from kv-server management seeds.
crowdb_svc_client_t crowdb_svc_client_create(const char *const *seeds, size_t num_seeds);

// Destroy a ServiceRegistryClient handle.
void crowdb_svc_client_destroy(crowdb_svc_client_t client);
