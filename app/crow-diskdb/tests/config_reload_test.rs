...
use crow_diskdb::bg_task::{BackgroundTask, BgCtx, Trigger};
use crow_diskdb::ddb_config::{CompactionConfig, DdbConfig, KeepAliveConfig};
...
fn make_keepalive(handle: &Arc<ArcSwap<DdbConfig>>) -> KeepAlive {
    let kv_client = Arc::new(CrowkvClient::new(ClientConfig::new(vec![
