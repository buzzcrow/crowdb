...
//! gauges to the crowdb-common `Gauge`/`Counter` reporting layer on a
...
/// crowdb-common `allocate_total`/`free_total` counters and recomputes
...
impl ReportingTask {
    ...
    fn tick(&self, ctx: &BgCtx) {
        ...
        for dg_id in dg_ids {
            if let Some(dg) = ctx.container.get_disk_group(dg_id) {
                ...
                for du in &u.disks {
                    ...
                    // Flush per-disk period counters into the crowdb-common totals.
        ...
        // Flush period deltas into the crowdb-common counters.
