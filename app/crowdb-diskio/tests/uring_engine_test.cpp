...
class TestDisk : public crowdb::diskio::Disk
{
  public:
    TestDisk(crowdb::diskio::DiskId id, const std::string &path, crowdb::diskio::IoEngine *engine)
        : id_(id),
    ...
    crowdb::diskio::Zone *find_zone(uint32_t zone_index) override
    {
        for (auto &z : zones_) {
...
{
    SUCCEED() << "UringEngine requires CROWDB_HAVE_LIBURING (Linux only)";
}
