...
//!
//! Key work: build N crowdb-rpc channels (the connection pool), spawn M
...
pub(crate) struct BenchConfig {
    ...
    /// Number of independent crowdb-rpc channels (1..=64). Default 4.
...
pub(crate) async fn run_bench<T: BenchTarget>(
    ...
    if cfg.flush_after_prepopulate && !flush_mgmt_urls.is_empty() {
        ...
        for url in &flush_mgmt_urls {
            match crowdb_console_shared::clients::http::ServerClient::new(url) {
