...
//!
//! Spawns the generic `BindingMonitor` (from `crow-kv-client`) with the
//! chunkdb range strategy as a leader-gated background task on this
//! `crow-kv-server`'s group-0 replica. Only the group-0 leader writes
...
//!
//! See `doc/design/chunkdb/design-crow-chunkdb-range-binding.md` §5
//! (Dynamic Binding Monitor).

use std::sync::Arc;

use crow_kv_client::{
    BindingMonitor, ChunkdbRangeStrategy, ClientConfig, CrowkvClient, ServiceRegistryClient,
};
use tracing::info;
