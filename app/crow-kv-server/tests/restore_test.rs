...
use crow_kv_server::restore::{group0_exists, scan_local_groups};
...
async fn scan_local_groups_multi_store() {
    ...
    assert_eq!(
        groups,
        vec![
            crow_kv_server::restore::LocalGroup {
                store_id: 0,
                group_id: 0
            },
            crow_kv_server::restore::LocalGroup {
...
async fn restart_restores_group0_from_disk() {
    ...
    let server = start_test_server_at(&root_path, &[], &[0])
        .await
        .expect("start crow-kv-server (first boot)");
    ...
    let server = start_test_server_at(&root_path, &[], &[0])
        .await
        .expect("start crow-kv-server (restart)");
    server
