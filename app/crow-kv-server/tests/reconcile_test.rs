...
use crow_kv_server::reconcile::{plan_reconcile, ReplicaRecord};
...
fn registry_with_group(gid: u64, rid: u64, remotes: &[(u64, &str, bool)]) -> Arc<KvStoreRegistry> {
    let registry = Arc::new(KvStoreRegistry::with_config(CrowKVConfig::for_tests()));
    ...
    if !remotes.is_empty() {
        group.set_remote_replicas(
            remotes
                .iter()
                .map(|(id, ep, v)| {
                    crow_kv::cluster::remote_replica::PxRemoteReplica::new(*id, (*ep).to_string())
                        .with_voting(*v)
                })
                .collect(),
        );
